...
function mockSignUpError(strapiError?: 'Auth.form.error.email.taken') {
  const spy = jest.fn()
  server.use(
    graphql.mutation('SignUp', (req, res, ctx) => {
      spy(req)
...
describe('Signup page', () => {
  ...
  it('Shows an error message if the email is already taken', async () => {
    ...
    const password = 'AV4l1dP4ssw0rd'
    mockUserQuery()
    const signUpSpy = mockSignUpError('Auth.form.error.email.taken')
    const { container } = render(<SignUp />, { wrapper: Providers })
    ...
    const errorMessage = await screen.findByText(/un compte avec cet email existe déjà/i)
    expect(errorMessage).toBeInTheDocument()
    const successMessage = await screen.queryByText(
      /le compte a été créé avec succès. veuillez valider votre e-mail en cliquant sur lien que vous allez recevoir dans votre boîte de réception./i
    )
